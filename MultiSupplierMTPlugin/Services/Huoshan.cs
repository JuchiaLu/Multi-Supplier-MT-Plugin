...
{
    public class Huoshan : MultiSupplierMTService
    {
        private class Sign
        {
            ...
            public async Task<HttpResponseMessage> Request(HttpMethod method, List<KeyValuePair<string, string>> queryList, byte[] body, string contentType,
                DateTimeOffset date, string action, string version, CancellationToken cToken)
            {
        ...
        {
            using (var form = new Forms.FormHuoshan(options, environment))
            {
        ...
        public override async Task<List<string>> TranslateAsync(MultiSupplierMTOptions options, List<string> texts, string srcLangCode, string trgLangCode, List<string> tmSources, List<string> tmTargets, MTRequestMetadata metaData, CancellationToken cToken)
        {
            ...
            HttpResponseMessage response = await sign.Request(HttpMethod.Post, new List<KeyValuePair<string, string>>(),
                    jsonRequestBytes, contentType: contentType, DateTimeOffset.Now, action, version, cToken);
